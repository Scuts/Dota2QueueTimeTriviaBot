...
const sources = [
    {
        ...
        transform: (respObj) => {
            ...
            Object.keys(scripts)
                .filter((key) => {
                return (!(key.includes("item_recipe") && scripts[key].ItemCost === "0") &&
                    key !== "Version");
            })
                .forEach((key) => {
                var item = {
                    ...replaceSpecialAttribs(strings[`DOTA_Tooltip_ability_${key}_Description`], scripts[key].AbilitySpecial, true, scripts[key]),
                };
                item.id = parseInt(scripts[key].ID);
                item.img = `/apps/dota2/images/items/${key.replace(/^item_/, "")}_lg.png?t=${1593393829403}`;
                ...
                item.attrib = formatAttrib(scripts[key].AbilitySpecial, strings, `DOTA_Tooltip_ability_${key}_`).filter((attr) => !attr.generated || attr.key === "lifetime");
    ...
    },
    {
        ...
        transform: (respObj) => {
            ...
            Object.keys(scripts)
                ...
                ability.desc = replaceSpecialAttribs(strings[`DOTA_Tooltip_ability_${key}_Description`], scripts[key].AbilitySpecial, false, scripts[key]);
                ability.dmg =
            ...
            for (let key of keys) {
                ...
                if (keyArr[0].toLowerCase() == "general") {
                    ...
                    if (itemName) {
                        ...
                        if (!result[patch].items.misc)
                            result[patch].items.misc = [];
                    ...
                    if (heroName) {
                        ...
                        if (!result[patch].heroes.misc)
                            result[patch].heroes.misc = [];
...
const dayDifference = Math.ceil(Math.abs(today.getTime() - new Date(lastPatch.date).getTime()) /
    (1000 * 3600 * 24));
patches.forEach((p, i) => {
...
${cfs
        .map((filename) => `${filename.split(".")[0]}: require(__dirname + '/build/${filename.split(".")[0]}.json')`)
        .join(",\n")}
};`;
